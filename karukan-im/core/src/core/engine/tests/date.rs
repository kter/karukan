...
#[test]
fn a_date_segment_records_nothing_while_its_neighbour_records() {
    // Learning is per segment, so a date must drop out of the record on
    // its own without taking the segment beside it with it.
    let settings = Settings::default();
    let mut engine = date_engine(settings.date.clone(), "きょう");
    engine.learning = Some(LearningCache::new(LearningConfig::default()));
    let date = engine
    ...
    engine.state = InputState::Conversion {
        ...
        segments: vec![
            ...
            Segment {
                reading: "はれ".to_string(),
                candidates: CandidateList::new(vec![Candidate {
                    text: "晴れ".to_string(),
                    reading: Some("はれ".to_string()),
    ...
    assert!(
        learning.lookup("きょう").is_empty(),
        "date segment leaked into the learning cache"
    );
    assert!(
        learning
            .lookup("はれ")
            .iter()
            .any(|(surface, _)| surface == "晴れ"),
        "the neighbouring segment was not recorded: {:?}",
        learning.lookup("はれ")
    );
}
