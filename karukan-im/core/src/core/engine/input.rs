...
impl InputMethodEngine {
    ...
    pub(super) fn process_key_composing(
        ...
        match key.keysym {
            ...
            // Shift+Tab bypasses the learning cache so stale history can be
            // escaped without changing ordinary Tab/Space conversion.
            Keysym::TAB if shift_active => self.start_conversion(LearningLookup::Skip),
            Keysym::ISO_LEFT_TAB => self.start_conversion(LearningLookup::Skip),
            Keysym::TAB => self.start_conversion(LearningLookup::Use),
