...
impl InputMethodEngine {
    ...
    pub(super) fn apply_candidate_filter(&mut self, next: CandidateSource) -> EngineResult {
        ...
        {
            *filter = Some(next);
            if let Some(segment) = segments.get_mut(*focus) {
                segment.candidates = list.clone();
            }
        }
        debug!("candidate filter → {:?}", next);
        let mut result = self.update_conversion_preedit();
        let aux = self.format_aux_conversion_with_page(&aux_reading, Some(&list));
        for action in &mut result.actions {
            if let EngineAction::UpdateAuxText(text) = action {
                *text = aux.clone();
            }
        }
        result
    ...
    fn model_source_view(&mut self, reading: &str) -> Vec<Candidate> {
        ...
        let context = self.segment_lctx(focus);
        self.model_candidates_with_context(reading, self.config.num_candidates, &context)
            .into_iter()
            .map(|text| Candidate {
                text,
                reading: Some(reading.to_string()),
