...
/// Once the user has split a conversion into multiple segments, candidates
/// may not extend beyond the segment reading.
fn retain_target_scoped_candidates(
    candidates: &mut Vec<AnnotatedCandidate>,
    reading: &str,
    drop_predictive: bool,
) {
    if drop_predictive {
...
impl InputMethodEngine {
    ...
    pub(super) fn focused_reading(&self) -> Option<&str> {
        match &self.state {
            InputState::Conversion {
                segments, focus, ..
            } => segments.get(*focus).map(|segment| segment.reading.as_str()),
            _ => None,
        }
    }

    /// Editor context followed by the selected surfaces of segments to the
    /// left of `index`.
    pub(super) fn segment_lctx(&self, index: usize) -> String {
        let base = self.truncate_context_for_api();
        let preceding: String = match &self.state {
            InputState::Conversion { segments, .. } => segments
                .iter()
                .take(index)
                .map(|segment| {
                    segment
    ...
    fn reconvert_segment(&mut self, index: usize) {
        let (reading, drop_predictive) = match &self.state {
        ...
        let context = self.segment_lctx(index);
        let mut candidates = self.build_conversion_candidates_with_context(
            &reading,
            &reading,
            "",
            self.config.num_candidates,
        ...
        retain_target_scoped_candidates(&mut candidates, &reading, drop_predictive);
        if candidates.is_empty() {
        ...
        let candidates = self.to_conversion_candidate_list(candidates, &reading);
        if let InputState::Conversion { segments, .. } = &mut self.state
            && let Some(segment) = segments.get_mut(index)
        {
            segment.candidates = candidates;
        }
        self.debug_assert_conversion_segments();
    }

    fn debug_assert_conversion_segments(&self) {
        ...
        } = &self.state
        {
            ...
            debug_assert!(*focus < segments.len());
            debug_assert_eq!(
                segments
                    .iter()
    ...
    pub(super) fn build_conversion_candidates(
        ...
        self.build_conversion_candidates_with_context(
            reading,
            base,
            pending,
            num_candidates,
    ...
    fn build_conversion_candidates_with_context(
        &mut self,
        reading: &str,
        base: &str,
        pending: &str,
        ...
        let candidates = match context {
            Some(context) => self.model_candidates_with_context(reading, num_candidates, context),
            None => self.model_candidates(reading, num_candidates),
        };
    ...
    pub(super) fn process_key_conversion(
        ...
        match key.keysym {
            ...
            Keysym::LEFT if shift_active => self.resize_focused_segment(-1),
            Keysym::RIGHT if shift_active => self.resize_focused_segment(1),
            Keysym::LEFT => self.move_focus(-1),
            Keysym::RIGHT => self.move_focus(1),
    ...
    /// Move the focused conversion segment. Boundary arrows are consumed so
    /// they never leak to the application.
    fn move_focus(&mut self, delta: i32) -> EngineResult {
        match &mut self.state {
            InputState::Conversion {
                ...
                let Some(next) = focus.checked_add_signed(delta as isize) else {
                    return EngineResult::consumed();
                };
                ...
                *focus = next;
                // A source view belongs to the segment it queried. Moving to
                // another segment returns to its ordinary candidate list.
                *filter = None;
            }
            _ => return EngineResult::not_consumed(),
        }
        self.update_conversion_preedit()
    }

    /// Move one reading character between the focused segment and its right
    /// neighbour, creating or deleting that neighbour at the boundary.
    fn resize_focused_segment(&mut self, delta: i32) -> EngineResult {
        let focus = match &self.state {
            InputState::Conversion { focus, .. } => *focus,
            _ => return EngineResult::not_consumed(),
        };
        ...
        match delta {
            -1 => {
                let transferred = {
                    ...
                    if segments[focus].reading.chars().count() <= 1 {
                        return EngineResult::consumed();
                    }
                    *filter = None;
                    let split_at = segments[focus]
                        .reading
                        .char_indices()
                        .next_back()
                        .map(|(index, _)| index)
                        .unwrap_or(0);
                    segments[focus].reading.split_off(split_at)
                };
                if let InputState::Conversion { segments, .. } = &mut self.state {
                    if let Some(next) = segments.get_mut(focus + 1) {
                        next.reading.insert_str(0, &transferred);
                    } else {
                        segments.push(Segment {
                            reading: transferred.clone(),
                            candidates: CandidateList::new(vec![Candidate::new(transferred)]),
                        });
                    }
                }
                self.reconvert_segment(focus);
                self.reconvert_segment(focus + 1);
            }
            1 => {
                let transferred = {
                    let InputState::Conversion {
                    ...
                    let Some(next) = segments.get_mut(focus + 1) else {
                        return EngineResult::consumed();
                    };
                    let Some(first_len) = next.reading.chars().next().map(char::len_utf8) else {
                        return EngineResult::consumed();
                    };
                    *filter = None;
                    next.reading.drain(..first_len).collect::<String>()
                };
                if let InputState::Conversion { segments, .. } = &mut self.state {
                    segments[focus].reading.push_str(&transferred);
                    if segments[focus + 1].reading.is_empty() {
                        segments.remove(focus + 1);
                    }
                }
                self.reconvert_segment(focus);
                if matches!(
                    &self.state,
                    InputState::Conversion { segments, .. } if focus + 1 < segments.len()
                ) {
                    self.reconvert_segment(focus + 1);
                }
            }
    ...
    /// Full selected text and each segment's `(reading, surface)` pair.
    ...
    /// composition.
    pub(super) fn finish_conversion(&mut self, _text: &str, selections: &[(String, String)]) {
    ...
    fn delete_selected_candidate_from_history(&mut self) -> EngineResult {
        ...
        let result = if let Some(source) = prev_filter {
            self.apply_candidate_filter(source)
        } else {
            self.update_conversion_preedit()
        };
        if let Some(candidates) = self.state.candidates_mut()
            && !candidates.is_empty()
        {
            candidates.set_cursor(prev_cursor);
            return self.update_conversion_preedit();
        }
        result
    }
    ...
    /// Update the multi-segment preedit after focus or candidate changes.
    pub(super) fn update_conversion_preedit(&mut self) -> EngineResult {
        self.debug_assert_conversion_segments();
        let (preedit, reading, candidates) = match &self.state {
            InputState::Conversion {
                segments, focus, ..
            } => {
                let mut caret = 0;
                let preedit_segments = segments
                    .iter()
                    .enumerate()
                    .map(|(index, segment)| {
                        let text = segment
                        ...
                        PreeditSegment::new(text, attribute)
                    })
                    .collect();
                (
                    Preedit::from_segments(preedit_segments, caret),
                    segments[*focus].reading.clone(),
                    segments[*focus].candidates.clone(),
                )
            }
            _ => return EngineResult::not_consumed(),
        };
