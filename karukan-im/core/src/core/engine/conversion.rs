...
impl InputMethodEngine {
    ...
    /// Character length of the settled reading — the string `split_target`
    /// and `target_len` are measured against.
    pub(super) fn settled_reading_len(&self) -> usize {
        self.input_buf
            .settled_reading(&self.converters.romaji)
            .chars()
            .count()
    }
    ...
    fn commit_partial_conversion(&mut self, text: &str) -> Option<EngineResult> {
        ...
        let candidates =
            self.build_conversion_candidates(&rest, &rest, "", self.config.num_candidates, false);
    ...
    fn delete_selected_candidate_from_history(&mut self) -> EngineResult {
        ...
        retain_target_scoped_candidates(
            &mut candidates,
            &reading,
            target_len < self.input_buf.char_count(),
        );
        if candidates.is_empty() {
