...
impl InputMethodEngine {
    ...
    pub(super) fn model_candidates(&mut self, reading: &str, num_candidates: usize) -> Vec<String> {
        ...
        self.model_candidates_with_context(reading, num_candidates, &base_ctx)
    }

    /// Model candidates with an explicit left context. Conversion segments
    /// append the selected surfaces of all preceding segments to the editor
    /// context before calling this variant.
    pub(super) fn model_candidates_with_context(
        &mut self,
        reading: &str,
        num_candidates: usize,
        base_ctx: &str,
