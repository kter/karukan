...
impl InputMethodEngine {
    ...
    pub(super) fn format_aux_conversion_with_page(
        ...
        if !self.config.verbose {
            return format!(
                "{header}{page_info}{target_info} {shown}{empty_note}{source_label}{delete_hint}"
            );
        }
        ...
        format!(
            "{header}{page_info}{target_info} {shown}{empty_note}{ctx} | {timing} | {model}{source_label}{delete_hint}"
        )
    }
