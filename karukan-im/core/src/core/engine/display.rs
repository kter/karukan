    ...
    pub(super) fn format_aux_conversion_with_page(
        ...
        let target_info = match &self.state {
            InputState::Conversion {
                segments, focus, ..
            } if segments.len() > 1 => format!(" {}/{}文節", focus + 1, segments.len()),
            _ => String::new(),
        };
        if !self.config.verbose {
            return format!(
                "{header}{page_info}{target_info} {reading}{empty_note}{source_label}{delete_hint}"
            );
        }
        let reading = self
        ...
        format!(
            "{header}{page_info}{target_info} {reading}{empty_note}{ctx} | {timing} | {model}{source_label}{delete_hint}"
        )
    }
